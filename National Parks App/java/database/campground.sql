...
SELECT DISTINCT *
FROM site
WHERE campground_id = ? AND site_id NOT IN (SELECT site.site_id FROM reservation
                                                        INNER JOIN site ON site.site_id = reservation.site_id
                                                        INNER JOIN campground ON campground.campground_id = site.campground_id
                                                        WHERE site.campground_id = ? AND ((? BETWEEN reservation.from_date AND reservation.to_date) OR (? BETWEEN reservation.from_date AND reservation.to_date)
...
INNER JOIN campground 
        ON site.campground_id = campground.campground_id
                WHERE site.campground_id = ? AND site_id
                NOT IN (SELECT site.site_id FROM site INNER JOIN reservation ON site.site_id = reservation.site_id
                        WHERE (? < reservation.from_date AND ? < reservation.from_date) OR (? > reservation.to_date AND ? > reservation.to_date));


SELECT DISTINCT * FROM site INNER JOIN campground ON site.campground_id = campground.campground_id INNER JOIN reservation ON site.site_id = reservation.site_id WHERE
							(? < reservation.from_date AND ? < reservation.from_date) OR (? > reservation.to_date AND ? > reservation.to_date)
